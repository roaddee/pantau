...
<?php if ($data['aktif'] == '0'): ?>
	<a href="<?= site_url("pelanggan/lock/{$data['id']}/{$data['aktif']}")?>" class="btn bg-navy btn-flat btn-sm" title="'Aktifkan" ><i class="fa fa-lock"></i></a>
<?php elseif ($data['aktif'] == '1'): ?>
	<a href="<?= site_url("pelanggan/lock/{$data['id']}/{$data['aktif']}")?>" class="btn bg-navy btn-flat btn-sm" title="Non-aktifkan"><i class="fa fa-unlock"></i></a>
<?php endif; ?>
